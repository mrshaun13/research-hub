        ...
        return (
          <div key={p.slug} className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-gray-400 truncate flex-1">{p.title}</span>
              <span className="text-[10px] text-gray-600">{formatDuration(total)}</span>
            </div>
            <div className="flex h-5 rounded overflow-hidden" style={{ width: maxTotal > 0 ? `${(total / maxTotal) * 100}%` : '100%', minWidth: '60px' }}>
                ...
                {!isCollapsed && catRows.map((row, ri) => (
                  <tr key={`${cat}-${ri}`} className="border-b border-gray-800/30 hover:bg-gray-900/30">
                    ...
                    {row.values.map((val, vi) => {
                      ...
                      return (
                        <td key={vi} className={`py-1.5 px-3 text-[11px] ${isBest ? 'text-emerald-400 font-semibold' : 'text-gray-400'}`}>
                          {formatted}
