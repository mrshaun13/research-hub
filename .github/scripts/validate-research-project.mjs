...
const MIN_DATA_FILE_SIZE = 200;       // bytes — a real data file has substance
...
const MAX_DURATION_MINUTES = 1440;    // 24 hours — sanity check
...
function parseRegistry(registryPath) {
  ...
  // This is a simplified approach — works for the known registry format
  try {
...
function validateStructure(report, projectDir, slug) {
  ...
  if (existsSync(componentsDir) && statSync(componentsDir).isDirectory()) {
    ...
    if (componentFiles.includes('Overview.jsx')) {
      ...
      report.fail('Structure', '`Overview.jsx` component missing — every research project needs an overview');
    }

    if (componentFiles.includes('Sources.jsx')) {
      report.pass('Structure', '`Sources.jsx` component present');
    } else {
      report.fail('Structure', '`Sources.jsx` component missing — research must cite sources');
    }
  } else {
  ...
  if (existsSync(dataDir) && statSync(dataDir).isDirectory()) {
    ...
    if (dataFiles.length > 0) {
      ...
      if (hasSubstantialData) {
        ...
        report.fail('Structure', `All data files are under ${MIN_DATA_FILE_SIZE} bytes — likely empty or placeholder`);
      }
    } else {
...
function validateRegistryEntry(report, entry, slug) {
  ...
  if (typeof entry.query === 'string' && entry.query.length >= MIN_QUERY_LENGTH) {
    ...
    report.fail('Registry', `Query is too short — real research needs a substantive prompt (min: ${MIN_QUERY_LENGTH} chars)`);
  }
...
function validateTelemetry(report, telemetry) {
  if (!telemetry || typeof telemetry !== 'object') {
    report.fail('Telemetry', 'No telemetry object found — research projects must include build telemetry');
    return;
  }
  ...
  if (typeof telemetry.searchesPerformed === 'number' && telemetry.searchesPerformed >= MIN_SEARCHES_PERFORMED) {
    ...
    report.fail('Telemetry', `Only ${telemetry.searchesPerformed} searches — real research requires more (min: ${MIN_SEARCHES_PERFORMED})`);
  }
  ...
  if (typeof telemetry.sourcesCount === 'number' && telemetry.sourcesCount >= MIN_SOURCES_COUNT) {
    ...
    report.fail('Telemetry', `Only ${telemetry.sourcesCount} sources — research must cite real sources (min: ${MIN_SOURCES_COUNT})`);
  }
  ...
  if (typeof telemetry.sectionsBuilt === 'number' && telemetry.sectionsBuilt >= MIN_SECTIONS_BUILT) {
    ...
    report.fail('Telemetry', `Only ${telemetry.sectionsBuilt} sections — dashboards need substance (min: ${MIN_SECTIONS_BUILT})`);
  }
  ...
  if (typeof telemetry.chartsBuilt === 'number' && telemetry.chartsBuilt >= MIN_CHARTS_BUILT) {
    ...
    report.fail('Telemetry', `Only ${telemetry.chartsBuilt} charts — research dashboards must have visualizations`);
  }
  ...
  if (typeof telemetry.dataPointsCollected === 'number' && telemetry.dataPointsCollected >= MIN_DATA_POINTS) {
    ...
    report.fail('Telemetry', `Only ${telemetry.dataPointsCollected} data points — research needs real data (min: ${MIN_DATA_POINTS})`);
  }
  ...
  if (telemetry.contentAnalysis && typeof telemetry.contentAnalysis === 'object') {
    ...
    if (missingCA.length === 0) {
      report.pass('Telemetry', `Content analysis present — ${ca.fleschKincaidLabel}, ${ca.bloomsLabel}, ${ca.totalWords} words`);
    } else {
  ...
  if (telemetry.hoursSaved && typeof telemetry.hoursSaved === 'object') {
    ...
    if (missingHS.length === 0) {
      report.pass('Telemetry', `Hours saved present — ${hs.totalHoursSaved}h total (${hs.equivalentLabel})`);
    } else {
  ...
  if (telemetry.consumptionTime && typeof telemetry.consumptionTime === 'object') {
    ...
    if (missingCT.length === 0) {
      report.pass('Telemetry', `Consumption time present — ${ct.estimatedLabel}`);
    } else {
  ...
  if (typeof telemetry.userPrompt === 'string' && telemetry.userPrompt.length >= MIN_QUERY_LENGTH) {
    ...
    report.fail('Telemetry', 'User prompt is too short — real research starts with a real question');
  }
  ...
  if (typeof telemetry.researchPlan === 'string' && telemetry.researchPlan.length >= 50) {
    ...
    report.fail('Telemetry', 'Research plan is too short — should contain the full checkpoint summary');
  }
}
...
function validateContentQuality(report, projectDir) {
  ...
  if (existsSync(appPath)) {
    ...
    if (/import\s+.*react/i.test(appContent) || /from\s+['"]react['"]/i.test(appContent)) {
      ...
      report.fail('Content', '`App.jsx` does not import React — not a valid React component');
    }
    ...
    if (/section|activeSection|setActive|sidebar|nav/i.test(appContent)) {
      ...
      report.warn('Content', '`App.jsx` may lack section navigation — research dashboards typically have internal nav');
    }
  }
  ...
  if (existsSync(componentsDir)) {
    ...
    if (hasChartImport) {
      ...
      report.fail('Content', 'No chart/visualization imports found — research dashboards must have data visualizations');
    }

    if (hasInsightCallout) {
      report.pass('Content', 'Components include insight callouts (research analysis patterns)');
    } else {
      report.warn('Content', 'No InsightCallout patterns found — research dashboards typically highlight key findings');
    }
  }
  ...
  if (existsSync(dataDir)) {
    ...
    if (hasExports) {
      ...
      report.fail('Content', 'Data files do not export anything — must use ES module exports');
    }

    if (totalDataSize >= 1000) {
      report.pass('Content', `Total data content: ${(totalDataSize / 1024).toFixed(1)}KB`);
    } else {
      report.fail('Content', `Total data content is only ${totalDataSize} bytes — research data should be substantial`);
    }
  }
}
...
function validateProductLensExtras(report, entry, telemetry) {
  ...
  report.pass('Product Lens', 'Product comparison project detected — running additional checks');
  ...
  if (typeof telemetry?.productsCompared === 'number' && telemetry.productsCompared >= 3) {
    ...
    report.fail('Product Lens', `Only ${telemetry.productsCompared} products compared — product comparisons need at least 3`);
  } else {
...
async function main() {
  ...
  let summary = '# 🔬 Research Project Validation Report\n\n';
